...
import { weekCallStats, periodCallStats, weekGesprekkenPerUnit, weekMailStats, periodCallStatsDaily, periodGesprekkenPerUnit, periodMailStats } from "@/data/tvData";
...
export function CallStats({ mode }: CallStatsProps) {
  ...
  if (mode === "period") {
    const totalOutbound = periodCallStatsDaily.reduce((s, d) => s + d.outbound, 0);
    const totalMinutes = periodCallStatsDaily.reduce((s, d) => s + d.duration, 0);
    const hours = Math.floor(totalMinutes / 60);
    const mins = totalMinutes % 60;
    const totalGesprekken = periodGesprekkenPerUnit.reduce((s, u) => s + u.gesprekken, 0);

    const combinedDailyData = periodCallStatsDaily.map((d, i) => ({
      day: d.day,
      outbound: d.outbound,
      mails: periodMailStats.perDay[i]?.mails ?? 0,
    }));
      ...
      <div className={cn("bg-card rounded-xl border border-border h-full overflow-hidden flex flex-col", compact ? "p-3" : "p-5")}>
        ...
        <div className={cn("grid grid-cols-4", compact ? "gap-2 mb-2" : "gap-4 mb-4")}>
          <div className="text-center">
            ...
            <p className={cn("font-bold text-foreground", compact ? "text-base" : "text-2xl")}>{totalOutbound.toLocaleString("nl-NL")}</p>
          ...
          <div className="text-center">
            ...
            <p className={cn("font-bold text-foreground", compact ? "text-base" : "text-2xl")}>{periodMailStats.totalAcquisitieMails}</p>
            <p className="text-xs text-muted-foreground">Acq. mails</p>
          </div>
        </div>
        <div className={cn("flex flex-wrap gap-2", compact ? "mb-1" : "mb-4")}>
          {periodGesprekkenPerUnit.map((u, i) => (
            <div key={u.unit} className="bg-muted/50 rounded-lg px-3 py-1.5 text-xs">
              ...
              <span className="font-semibold text-foreground">{periodMailStats.perUnit[i]?.mails ?? 0} mails</span>
            </div>
          ))}
        </div>
        {/* Chart legend */}
