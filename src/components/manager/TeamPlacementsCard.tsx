...
import { teamStats, teamPlacements, teamPlacementsChartData, PlacementRecord } from "@/data/managerData";
import { myTeamConsultants } from "@/data/managerData";

interface TeamPlacementsCardProps {
  delay?: number;
}

export function TeamPlacementsCard({ delay = 0 }: TeamPlacementsCardProps) {
  const { ref, isVisible } = useAnimateOnMount({ delay: delay + 400 });
  const [visibleCount, setVisibleCount] = useState(10);
  const [isLoading, setIsLoading] = useState(false);
  const scrollRef = useRef<HTMLDivElement>(null);
  
  const activePlacements = teamPlacements.filter(p => p.isActive);
  ...
  const getConsultantName = (consultantId: number) => {
    const consultant = myTeamConsultants.find(c => c.id === consultantId);
      ...
      <div className="bg-card rounded-xl p-5 border border-border group flex flex-col h-full">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-medium text-foreground">Team Plaatsingen</h3>
              <p className="text-xs text-muted-foreground mt-0.5">Mijn team prestaties</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-end gap-6 mb-4">
          <div>
            <AnimatedNumber 
              value={teamStats.totalPlacements} 
              delay={delay + 300}
          ...
          <div>
            <AnimatedNumber 
              value={teamStats.activePlacements} 
              delay={delay + 400}
