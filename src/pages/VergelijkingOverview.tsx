...
const VergelijkingOverview = () => {
  ...
  const otherMembers = useMemo(
    () => teamMembers.filter((m) => !m.isCurrentUser),
    []
  );
  ...
  const selectedMember = useMemo(
    () => otherMembers.find((m) => m.id === selectedMemberId),
    [selectedMemberId, otherMembers]
  );
        ...
        <main className="flex-1 p-6 overflow-auto">
          ...
          <div className="mb-6">
            ...
            <p className="text-sm text-muted-foreground">
              Vergelijk jouw activiteiten en resultaten met een teamlid
            </p>
          </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {positiveGaps.length > 0 ? (
                  positiveGaps.map((gap, i) => (
                    <span key={i} className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-emerald-500/10 text-emerald-500 text-xs font-medium">
                      {gap.icon} {gap.label}
                    </span>
                  ))
                ) : (
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {negativeGaps.length > 0 ? (
                  negativeGaps.map((gap, i) => (
                    <span key={i} className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-red-500/10 text-red-500 text-xs font-medium">
                      {gap.icon} {gap.label}
                    </span>
                  ))
                ) : (
