          ...
          <Card className="h-full">
            ...
            <CardContent>
              ...
              <DevNote
                ...
                source={<><code>getTotalStartupInvestment()</code> in <code>groeimodelData.ts</code>.</>}
                logic={<>Sum of <code>result.startupCost</code> across all consultant lifecycles, where <code>startupCost = |min(cumulativeBalance)|</code> reached before break-even (or the total cumulative loss if break-even was never reached).</>}
              />
            </CardContent>
          </Card>
        </AnimatedCard>
          ...
          <Card className="h-full">
            ...
            <CardContent>
              ...
              <DevNote
                ...
                source={<><code>getAverageBreakEvenMonths()</code> in <code>groeimodelData.ts</code>.</>}
                logic={<>Arithmetic mean of <code>result.breakEvenMonth</code> across consultants where <code>breakEvenMonth !== null</code>. Consultants still in startup phase or terminated before break-even are excluded.</>}
              />
            </CardContent>
          </Card>
        </AnimatedCard>
        ...
        <AnimatedCard delay={160}>
          <Card className="h-full">
            ...
            <CardContent>
              ...
              <DevNote
                story={<><strong>As a user (C-level)</strong>, I want to see how many active consultants have not yet reached break-even, <strong>so that</strong> I understand the size of the current open investment position and how much margin pressure exists from non-profitable hires.</>}
                source={<><code>getActiveStartupCount()</code> in <code>groeimodelData.ts</code>.</>}
                logic={<>Count of lifecycles where <code>endDate === undefined</code> (still employed) AND the consultant's <code>cumulativeBalance</code> at the most recent month is still &lt; 0 (i.e. break-even not yet reached).</>}
              />
            </CardContent>
          </Card>
        </AnimatedCard>
        ...
        <AnimatedCard delay={240}>
          <Card className="h-full">
            ...
            <CardContent>
              ...
              <DevNote
                story={<><strong>As a user (C-level)</strong>, I want to see the multiple of return that the consultant cohort has generated relative to the capital invested in their startup phase, <strong>so that</strong> I can judge whether our hiring + ramp-up model is financially healthy at the portfolio level.</>}
                source={<><code>getCohortROI()</code> in <code>groeimodelData.ts</code>.</>}
                logic={<><code>ROI = Σ profitSinceBreakEven / Σ startupCost</code> across all consultant lifecycles. A value of 2.00× means every euro invested in ramp-up has earned two euros in post break-even profit.</>}
              />
            </CardContent>
          </Card>
        </AnimatedCard>
