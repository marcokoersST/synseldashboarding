...
function FunnelDropOff({ data }: { data: { stage: string; value: number; dropLabel: string }[] }) {
  const W = 900;
  const H = 240;
  const padX = 20;
  const padTop = 30;
  ...
  return (
    <div className="w-full">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ height: H }}>
        {/* Trapezoidal drop-off connectors */}
        {data.map((d, i) => {
          if (i === 0) return null;
          const prev = data[i - 1];
          ...
          return (
            <g key={`drop-${i}`} className="group cursor-pointer">
              <polygon
                ...
                fill={MARKETING_COLORS[0]}
                fillOpacity={0.18}
                className="transition-opacity duration-200 group-hover:opacity-100"
                style={{ opacity: 0.55 }}
              />
              <g className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
                <rect x={midX - 28} y={midY - 12} width={56} height={22} rx={6} fill="hsl(var(--foreground))" />
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={d.stage}>
              <rect x={x} y={y} width={barW} height={h} rx={6} fill={MARKETING_COLORS[0]} />
