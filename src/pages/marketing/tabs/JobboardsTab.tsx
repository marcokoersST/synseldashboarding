...
const JobboardsTab = ({ dateRange }: Props) => {
  ...
  const kpis = useMemo(() => {
    ...
    const items: { label: string; value: number; delta: number | null; format?: "currency"; invertDelta?: boolean }[] = [
      ...
      { label: "CPR", value: cpr, delta: deltaPercent(cpr, prevCpr), format: "currency", invertDelta: true },
    ];
    return items;
  }, [grand, grandCpr]);
            ...
            <table className="w-full caption-bottom text-sm table-fixed">
              <thead className="[&_tr]:border-b">
                <tr className="border-b transition-colors">
                  <th className="h-12 px-4 text-left align-middle font-medium text-muted-foreground sticky top-0 bg-background z-10 w-[28%]">Jobboard / Categorie</th>
                      ...
                      {isOpen && parent.children.map((child) => {
                        ...
                        return (
                          <tr key={`${parent.board}-${child.category}`} className="border-b transition-colors bg-muted/20">
                            <td className="p-4 pl-10 align-middle text-muted-foreground w-[28%]">{child.category}</td>
